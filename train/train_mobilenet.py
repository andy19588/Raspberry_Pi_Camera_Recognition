...
def main():
    ...
    # 前處理
    X_train = tf.keras.applications.mobilenet_v2.preprocess_input(X_train.astype(np.float32))
    X_test = tf.keras.applications.mobilenet_v2.preprocess_input(X_test.astype(np.float32))
    
    y_train_cat = to_categorical(y_train, num_classes=3)
    y_test_cat = to_categorical(y_test, num_classes=3)

    print("\n=== 步驟 2: 建立與訓練 MobileNet 模型 ===")
    ...
    model_path = os.path.join(demo_dir, 'rps_mobilenet_model.h5')
